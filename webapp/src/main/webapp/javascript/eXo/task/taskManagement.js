...
//
	...
	TaskManagement.prototype.intCharData = function(projectId) {
	    var uri = eXo.task.TaskManagement.restContext + "/taskManager/task/tasksInfo?" +
		  "projectId=" + projectId;
			$.ajax({url: uri,
				   async: false,
				   success: function(result, status, xhr) {
					 //alert(1);
				     //location.reload();
					   eXo.task.TaskManagement.ganttData = result;
			       },
			       error: function() {
			    	 //alert(2);
		    	     //location.reload();
...
//		eXo.task.TaskManagement.ganttData = [
//         {
...
//       ];
	...
	TaskManagement.prototype.initDataChartForm = function(projectId) {
		eXo.task.TaskManagement.intCharData(projectId);

	    
	    $("#ganttChart").ganttView({ 
	        data: eval(eXo.task.TaskManagement.ganttData),
	        slideWidth: 900,
	        behavior: {
